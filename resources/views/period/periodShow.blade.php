...
@section ('content')
    ...
    <h3> Vous devez atteindre <?php echo($game->objectivePrice)?> euros </h3>
                ...
                <?php   }else{ ?>
                          <a href="{{ route('period.edit', $game->idGame)}}">
                            <div id="newPeriod">Commencer la prochaine periode </div>
  ...
  <div class ="row">
    ...
    @foreach($periods as $p)
      <?php if($p->idPeriod == $id){ ?>
      <p> <?php if ($p->isSold == 0 && $p->quantite == 0 ) {echo("Vous n'avez pas vendu votre récolte.");} else{echo"Vous avez vendu votre récolte.";} ?> </p>
      <p> Vous
        <?php if ($p->contratPosition == 0) {echo(" n'avez rien fait ");}
          else {
            if ($p->contratPosition > 0)
              {echo(" avez acheté ".$p->contratPosition." contrat");if($p->contratPosition > 1){echo("s");}}
            else
              {echo(" avez vendu ".$p->contratPosition*(-1)." contrat");if($p->contratPosition < -1){echo("s");}}
              {echo(" et  ".$p->quantite." tonnes sur le marché physique");}
           } ?>
      </p>
  ...
  <div class ="row">
    ...
    <p> Prix feme de cette période : <?php echo($bilan->priceSpotPeriod);?> </p>
...
<div style="width:400px;height:400px">
      <h3> Graphique synthèse de la periode  @foreach($periods as $p)
        <?php if($p->idPeriod == $id){
          echo($p->numberPeriod);
         } ?>
      @endforeach </h3>
